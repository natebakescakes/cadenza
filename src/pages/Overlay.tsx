...
function ReminderView({ hint, fadeMs }: { hint: CoachingHint; fadeMs: number }) {
...
function OptionsView({ hint, fadeMs }: { hint: CoachingHint; fadeMs: number }) {
        ...
        <>
          ...
          <div className="px-3 pt-2 pb-2.5 space-y-1.5">
            ...
            {alternatives.map((alt, i) => (
              ...
              >
                {/* De-emphasise alt combos slightly vs. primary */}
  ...
  return (
    ...
    <div className="flex h-screen w-screen items-end justify-start bg-transparent p-1">
      ...
      >
        {visible && hint && (
          optionsMode ? (
            <OptionsView key={hint.id} hint={hint} fadeMs={fadeMsRef.current} />
          ) : (
            <ReminderView key={hint.id} hint={hint} fadeMs={fadeMsRef.current} />
          )
        )}
      </AnimatePresence>
    </div>
